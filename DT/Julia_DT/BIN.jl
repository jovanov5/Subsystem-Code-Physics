    ...
    for r_x  = 0:L-1
        for r_y = 0:L-1
            if r_x == 0 && r_y == 0
                ...
                f_representative = get_f_representative((r_x, r_y), system)
                size_of_bff = 0
                dim_source, _ = zassenhaus_alg(e_representative, e_deformator, stabilizerview(state), nbits)
                dim_norm, _ = zassenhaus_alg(e_deformator, stabilizerview(state), nbits)
    ...
    for r_x  = 0:L-1
        for r_y = 0:L-1
            if r_x == 0 && r_y == 0
                ...
                e_representative = get_e_representative((r_x, r_y), system)
                size_of_bff = 0
                dim_source, _ = zassenhaus_alg(e_representative, e_deformator, stabilizerview(state), nbits)
                dim_norm, _ = zassenhaus_alg(e_deformator, stabilizerview(state), nbits)
                corr_function[r_x+1, r_y+1] = dim_source - dim_norm
