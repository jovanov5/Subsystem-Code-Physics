...
function visualise_the_stabiliser(stabiliser::PauliOperator, system::EdgeSquareLattice)
    ...
    # plot!([(0, 0), (L, 0)], color=:black, legend= false, xticks= 0:1:L, yticks= 0:1:L)
    # plot!([(0, 8), (8, 8), (8, 0)], color=:orange, legend= false, xticks= 0:1:L, yticks= 0:1:L)

    for i in 0:L-1
        for j in 0:L-1
            if stab[edge_picker(i, j, 1)] == 1 && stab[edge_picker(i, j, 1)+nbits] == 0
                plot!([(i, j), (i+1, j)], color=:blue, linewidth=4)
                # plot!([(i+1/2, j+1/2), (i+1/2, j-1/2)], color=:blue, linestyle=:dash, linewidth=2)
            elseif stab[edge_picker(i, j, 1)] == 0 && stab[edge_picker(i, j, 1)+nbits] == 1
                plot!([(i, j), (i+1, j)], color=:red, linewidth=4)
            elseif stab[edge_picker(i, j, 1)] == 1 && stab[edge_picker(i, j, 1)+nbits] == 1
                plot!([(i, j), (i+1, j)], color=:green, linewidth=4)
            end
            if stab[edge_picker(i, j, 0)] == 1 && stab[edge_picker(i, j, 0)+nbits] == 0
                plot!([(i, j), (i, j+1)], color=:blue, linewidth=4)
                # plot!([(i+1/2, j+1/2), (i-1/2, j+1/2)], color=:blue, linestyle=:dash, linewidth=2)
            elseif stab[edge_picker(i, j, 0)] == 0 && stab[edge_picker(i, j, 0)+nbits] == 1
                plot!([(i, j), (i, j+1)], color=:red, linewidth=4)
            elseif stab[edge_picker(i, j, 0)] == 1 && stab[edge_picker(i, j, 0)+nbits] == 1
                plot!([(i, j), (i, j+1)], color=:green, linewidth=4)
            end
        end
    end
...
function zassenhaus_map_f(g::PauliOperator)
    return g ⊗ g ⊗ g ⊗ g
end

function zassenhaus_map_h(g::PauliOperator)
    e = g * g # this is the identity operator, cool trick 
    return g ⊗ g ⊗ e ⊗ e
end

function zassenhaus_state(deformator, probe_state, nbits::Int)
...
function zassenhaus_state(charge_representative, deformator, probe_state, nbits::Int)
    """Given two sungroup of the Pauli group, given as invalid stabilisers, we return the "Zassenhauser" state.
    """
    state = maximally_mixed_state(4*nbits)
...
# this is the same as belo but with trivial representative
function zassenhaus_alg(deformator, probe_state, nbits::Int)
    ...
    """
    state = zassenhaus_state(deformator, probe_state, nbits)
    N = nbits
...
function zassenhaus_alg(charge_representative, deformator, probe_state, nbits::Int)
    """Given two sungroup of the Pauli group, given as invalid stabilisers generators, we return the dims we need.
    """
    state = zassenhaus_state(charge_representative, deformator, probe_state, nbits)
    N = nbits
    dim_intersection = 2*N - entanglement_entropy(state, (2*N+1):4N, Val(:clip))
    dim_sum = 4*N - dim_intersection - entanglement_entropy(state, 1:4N, Val(:clip))
    return dim_intersection, dim_sum
