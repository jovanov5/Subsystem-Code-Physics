...
# This simulation takes each point in the phase diagram and calculates all observables!
# TEE, mid-point EE, Zassenhaus correlators for the longest separation!
# What about stabilisation of the critical phase? Not now!
...
function main(L::Integer, d::Integer, p_f::Float64, p_b::Float64, t_mmt::Array{Int}, exp_index::Integer, debug::Int)
    ...
    description = "Singel Point in Phase Diagram. Full Scan of relevant order parameters: TEE, EE, BB, FF, oBB half sys."
    ...
    Boson_Boson = zeros(Int, n_t);
    Fermion_Fermion = zeros(Int, n_t);
    ...
    other_boson_deformator = get_m_deformator(system);
    r_x = Int(round(L/2));
    r_y = Int(round(L/2));
    r_2 = (r_x, r_y);
    ...
    for t_index = 1:n_t
        ...
        Boson_Boson[t_index] = particular_zassenhausen_correlator(r_2, state, system, get_e_reprentative, boson_deformator);
    ...
    if debug == 1
        ...
        println("Boson_Boson: ", Boson_Boson[end])
